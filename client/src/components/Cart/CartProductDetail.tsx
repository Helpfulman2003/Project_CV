...
const CartProductDetail = ({data, setOpen}: IProps) => {
  ...
  return (
    <div className='fixed top-0 left-0 w-full h-full bg-[#00000030] z-10'>
        <div className='bg-white z-20 overflow-y-auto rounded-md absolute w-[60%] h-[75%] opacity-100 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] p-4 grid grid-cols-2'>
            <div className=''>
                <div className=''>
                    <img src={data.images?.url[0].url} alt="" className='w-full object-cover'/>
                </div>
                <div className='flex items-center justify-start gap-4 mt-4'>
                    <div>
                        <img src={data.shopId?.avatar.url[0].url} alt="data.shop.shop_avatar.url" className='w-[50px] h-[50px] rounded-full' />
                    </div>
                    <div>
