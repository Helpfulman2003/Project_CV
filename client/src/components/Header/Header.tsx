...
import { removeFromWishList } from '../../redux/wishListSlice.ts';
...
const Header = () => {
    ...
    const handleDeleteWishList = (item: currentProduct) => {
    ...
    return (
        <div className='w-full px-[10%] py-3 bg-[#fff5e1]'>
            <div className='flex items-center justify-between'>
                ...
                <Link to='/'>
                    <div className=''>
                        <img src={Logo} alt="" className='lg:w-[80px] w-[100px] object-cover' />
                    </div>
                </Link>
                <div className='w-[60%] relative lg:block hidden' >
                ...
                <div className='block lg:flex lg:items-center lg:justify-between md:gap-4'>
                    <div className='relative lg:block hidden' onClick={() => setOpenWishlist(true)}>
                        <IoMdHeartEmpty size={30} />
                        <div className='absolute top-0 right-[-2px] rounded-full flex items-center justify-center w-4 h-4 bg-[#3bc177]'>
                            <span className='text-white text-[12px]'>{wishList && wishList.length}</span>
                        </div>
                    </div>
                    <div className='relative cursor-pointer' onClick={() => setOpenCart(true)} >
                        <IoCartOutline size={30} />
                        <div className='absolute top-0 right-[-2px] rounded-full flex items-center justify-center w-4 h-4 bg-[#3bc177]'>
                            <span className='text-white text-[12px]'>{cart && cart.length}</span>
                        </div>
                    </div>
            ...
            </div>
            {
                openCart && (
                    <div className='fixed w-full h-full top-0 left-0 bg-[#00000030] z-10'>
                        <div className='absolute top-0 lg:left-[75%] lg:w-[25%] md:left-[50%] md:w-[50%] left-[25%] w-[75%] bg-white h-full z-20'>
                ...
                openWishlist && (
                    <div className='fixed w-full h-full top-0 left-0 bg-[#00000030] z-10'>
                        <div className='absolute top-0 left-[75%] w-[25%] bg-white h-full z-20'>
                            <div className='w-full flex justify-end pt-5 px-4 cursor-pointer'>
                                <IoClose size={30} color='#333' onClick={() => setOpenWishlist(false)} />
                            </div>
                            <div className='overflow-y-auto h-[80%]'>
                                <div className='flex justify-start gap-3 items-center px-4 mb-2 pb-4 border-solid border-b border-gray-200'>
                                    <IoMdHeartEmpty size={24} />
                                    <span className='font-bold text-gray-800 text-[20px] \'>{wishList && wishList.length} items</span>
                                </div>
                                <div className=''>
                                    {
                                                ...
                                                <div key={item._id} className='mb-2'>
                                                    <div className='flex items-start justify-start px-4 gap-2 '>
                                                        ...
                                                        <div>
                                                            ...
                                                            <span className='font-bold text-red-500'>
                                                                {item?.discountPrice ?? 0} VND
                                                            </span>
                                                        </div>
                                                        <div className='flex-1 flex items-center justify-end'>
                                                            <IoClose size={20} className='cursor-pointer' onClick={() => handleDeleteWishList(item)} />
                                                        </div>
                                                    </div>
                                                </div>
                                            )
